...
# → Khi dev local/setup.ps1: dùng localhost + port runtime
...
Đọc `FIRESAFE_PRESET_CAMERA_*` từ `backend/.env.local` thông qua `setup.ps1`, rồi tạo sẵn một camera RTSP nếu URL khác rỗng. Nếu camera cùng RTSP URL đã tồn tại thì không tạo trùng. File migration không seed camera fake nữa.
