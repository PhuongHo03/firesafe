...
└── src/
    ...
    │   │   │   ├── PresetCameraSeeder.java          ← [Dev] Seed camera RTSP làm sẵn từ backend/.env.local
    ...
    │               └── V2__seed_data.sql            ← Seed roles + admin user
    │
    └── test/
        └── java/com/firesafe/backend/
...
Runtime manager tự chọn port runtime, chuẩn bị Java 21 nếu cần, start Docker infra rồi chạy backend bằng Maven Wrapper. Port thực tế xem trong:
...
3. Kiểm tra Redis key "alert:debounce:{camera_id}"
   ...
   │     → Publish alertId lên RabbitMQ → NotificationWorker gửi Telegram nếu bật
...
Đọc `FIRESAFE_PRESET_CAMERA_*` từ `backend/.env.local` thông qua runtime manager, rồi tạo sẵn một camera RTSP nếu URL khác rỗng. Nếu camera cùng RTSP URL đã tồn tại thì không tạo trùng. File migration không seed camera fake nữa.
