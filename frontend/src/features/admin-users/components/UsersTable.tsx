      ...
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ borderBottom: "1px solid var(--border)" }}>
            {["User", "Email", "Trạng thái", "Tạo lúc"].map(h => <th key={h} style={th}>{h}</th>)}
          </tr>
        </thead>
        <tbody>
          {loading ? (
            <tr><td colSpan={4} style={emptyTd}>Đang tải...</td></tr>
          ) : users.length === 0 ? (
            <tr><td colSpan={4} style={emptyTd}>Chưa có user</td></tr>
          ) : users.map(user => {
