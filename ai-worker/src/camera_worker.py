...
class CameraWorker:
    def __init__(self, config: CameraWorkerConfig, source: SharedRtspSource):
        self.config = config
        self._source = source
        self._stop_event = threading.Event()
    ...
    def _detect_loop(self):
        ...
        while not self._stop_event.wait(self.config.detection_interval_seconds):
            ...
            inference_ms = (time.monotonic() - inference_started_at) * 1000
            ...
            image_url = storage.upload(self.config.camera_id, detection.label, encode_png(annotated_frame))
            backend.create_alert(self.config.camera_id, detection.label, detection.confidence, image_url)
