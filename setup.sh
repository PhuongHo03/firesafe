...
MONITORING_DIR="$PROJECT_ROOT/monitoring-service"
...
cap_log_file() {
  ...
  if tail -n "$LOG_TAIL_LINES" "$log_path" > "$tmp_path" && mv "$tmp_path" "$log_path"; then
    return 0
  fi
  rm -f "$tmp_path"
  echo "Skipping locked/busy log file: $log_path" >&2
  return 0
}
...
trim_runtime_logs() {
  local log_path
  shopt -s nullglob
  for log_path in "$LOGS_DIR"/*.log; do
    cap_log_file "$log_path"
  done
  shopt -u nullglob
}
...
get_process_from_pid_file() {
  ...
  if [[ "$metadata_name" != "$name" || "$metadata_working_directory" != "$expected_working_directory" ]]; then
    echo "Ignoring invalid PID metadata for $name; will scan running processes." >&2
    return 1
  fi

  if [[ -n "$metadata_start_ticks" && -n "$current_start_ticks" && "$metadata_start_ticks" != "$current_start_ticks" ]]; then
    echo "Ignoring stale $name PID $raw_pid; PID was reused by another process." >&2
    return 1
  fi

  if [[ -n "$current_cwd" && "$current_cwd" != "$expected_working_directory" ]]; then
    echo "Ignoring stale $name PID $raw_pid; process is not running in $expected_working_directory." >&2
    return 1
  fi
...
set_runtime_ports() {
  ...
  if [[ ${#PORTS[@]} -eq 0 ]]; then
    ...
    PORTS[MONITORING_PORT]="$(get_free_port 8091)"
...
NEXT_PUBLIC_MONITORING_URL=http://localhost:${PORTS[MONITORING_PORT]}
EOF
...
start_logged_process() {
  ...
  (
    ...
    bash -lc "$command_text" 2>&1 | while IFS= read -r line; do
      append_capped_log "$log_path" "$line"
    done
  ) &
...
start_frontend() {
  ...
  local command_text="export NO_COLOR=1; export FORCE_COLOR=0; export TERM=dumb; export NEXT_TELEMETRY_DISABLED=1; export NEXT_PUBLIC_API_URL=http://localhost:${PORTS[BACKEND_PORT]}; export NEXT_PUBLIC_AI_WORKER_URL=http://localhost:${PORTS[AI_WORKER_PORT]}; export NEXT_PUBLIC_MONITORING_URL=http://localhost:${PORTS[MONITORING_PORT]}; npm run dev -- --port ${PORTS[FRONTEND_PORT]}"
  local pid
...
start_ai_worker() {
  ...
  local command_text="if [ ! -d venv ]; then $PYTHON_CMD -m venv venv; fi; ./venv/bin/python -m pip --disable-pip-version-check install -r requirements.txt >/dev/null; export BACKEND_URL=http://localhost:${PORTS[BACKEND_PORT]}; export MINIO_URL=localhost:${PORTS[MINIO_API_PORT]}; export PYTHONUNBUFFERED=1; ./venv/bin/python -u service.py --port ${PORTS[AI_WORKER_PORT]} --backend-url http://localhost:${PORTS[BACKEND_PORT]} --minio-url localhost:${PORTS[MINIO_API_PORT]}"
  local pid
...
start_monitoring_service() {
  local running status=0
  running="$(get_process_from_pid_file "monitoring-service" "$MONITORING_DIR")" || status=$?
  if [[ "$status" -eq 2 ]]; then
    exit 1
  fi
  if [[ -n "$running" ]]; then
    echo "Monitoring service already running (PID $running); keeping existing process."
    return
  fi

  local monitoring_log="$LOGS_DIR/monitoring-service.log"
  local command_text="if [ ! -d venv ]; then $PYTHON_CMD -m venv venv; fi; ./venv/bin/python -m pip --disable-pip-version-check install -r requirements.txt >/dev/null; export PYTHONUNBUFFERED=1; ./venv/bin/python -u service.py --port ${PORTS[MONITORING_PORT]} --backend-url http://localhost:${PORTS[BACKEND_PORT]} --ai-worker-url http://localhost:${PORTS[AI_WORKER_PORT]} --redis-port ${PORTS[REDIS_PORT]} --rabbitmq-url http://localhost:${PORTS[RABBITMQ_UI_PORT]} --minio-url localhost:${PORTS[MINIO_API_PORT]}"
  local pid
  pid="$(start_logged_process "monitoring-service" "$MONITORING_DIR" "$monitoring_log" "$command_text")"
  echo "Monitoring service starting (PID $pid); logs: .runtime/logs/monitoring-service.log"
}
...
start_infra() {
  ...
  docker compose -f "$COMPOSE_FILE" up -d | while IFS= read -r line; do
    echo "$line"
    append_capped_log "$docker_log" "$line"
  done
  append_capped_log "$docker_log" ""
  if wait_infra_ready 90; then
    append_capped_log "$docker_log" "=== Docker Compose Status: all services running/healthy ==="
  else
    append_capped_log "$docker_log" "=== Docker Compose Status: TIMEOUT waiting for all services healthy ==="
  fi
  docker compose -f "$COMPOSE_FILE" ps | while IFS= read -r line; do
    append_capped_log "$docker_log" "$line"
  done
  append_capped_log "$docker_log" ""
  append_capped_log "$docker_log" "=== Docker Compose Logs ==="
  docker compose -f "$COMPOSE_FILE" logs --no-color | remove_ansi_codes | while IFS= read -r line; do
    append_capped_log "$docker_log" "$line"
  done
}
...
runtime_process_ids() {
  ...
  case "$name" in
    backend)
      dir="$BACKEND_DIR"
      pattern='(spring-boot:run|mvnw)'
      ;;
    frontend)
      dir="$FRONTEND_DIR"
      pattern='(next[[:space:]]+dev|\.next)'
      ;;
    ai-worker)
      dir="$AI_WORKER_DIR"
      pattern='service\.py'
  ...
  ps -eo pid=,args= | awk -v dir="$dir" -v pattern="$pattern" '$0 ~ dir && $0 ~ pattern { print $1 }'
}

stop_runtime_process() {
  local name="$1"
  local expected_working_directory="$2"
  local pid status=0 stopped=0 fallback_pid
  ...
  for fallback_pid in $(runtime_process_ids "$name"); do
    if [[ -n "$pid" && "$fallback_pid" == "$pid" ]]; then
      continue
    fi
    echo "Stopping $name fallback process (PID $fallback_pid)..."
    stop_process_tree "$fallback_pid"
    stopped=1
  done
...
case "$COMMAND" in
  ...
  down)
    ensure_runtime_dirs
    stop_runtime_process "monitoring-service" "$MONITORING_DIR"
